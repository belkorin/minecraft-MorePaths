...
public class OtherPathBlock extends GrassPathBlock {
	...
	private static Properties getProperties(Block pathedFrom) {
		...
		Properties newPathProperties = AbstractBlock.Properties
				.create(state.getMaterial(), state.getMaterialColor(null, null))
	...
	public BlockState getStateForPlacement(BlockItemUseContext context) {
		return !this.getDefaultState().isValidPosition(context.getWorld(), context.getPos())
				? Block.nudgeEntitiesWithNewState(this.getDefaultState(), _pathedFrom.getDefaultState(),
	...
	private boolean checkIfFalling(IWorld worldIn, BlockPos currentPos) {
		...
		return worldIn.isAirBlock(currentPos.down())
				|| FallingBlock.canFallThrough(worldIn.getBlockState(currentPos.down())) && currentPos.getY() >= 0;
	}
